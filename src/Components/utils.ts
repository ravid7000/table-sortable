...
export const isColumnSortable = (columnIdx: number, options: PartialOptions, colSorting: boolean) => {
  if (colSorting === false || !options.sorting) {
    return false;
  }

  return options.sorting === true || isArray(options.sorting) && options.sorting.indexOf(columnIdx) > -1;
}

export const orderedColumns = (columns: PartialOptions["columns"]) => {
  return isArray(columns) ? columns.map(col => ({
    dataKey: col.dataKey,
    type: col.type || 'text',
  })) : [];
}
...
export const paginatedData = (data: Collection, options: PartialOptions, currentPage = 0) => {
  const rowsPerPage = (options.rowsPerPage || 10);
  const min = currentPage * rowsPerPage;
  const max = min + rowsPerPage;
  console.log({min, max, currentPage, rowsPerPage})
  return {
...
export const createPagination = ({ currentPage, totalPages, paginationWindow = 5 }: { currentPage: number, totalPages: number, paginationWindow?: number }) => {
  let pages = Array(paginationWindow).fill(0).map((_, idx) => idx + 1);
  ...
  if (currentPage <= totalPages && currentPage > 0) {
    ...
    if (currentPage > paginationWindow / 2 && currentPage < totalPages - paginationWindow / 2) {
